...
export async function GET(request: Request) {
  try {
    ...
    const [profiles, overview, leaderboard, adherence] = await Promise.all([listProfiles(), getGroupOverview(date), getSevenDayDeficitLeaderboard(date), getGroupAdherenceHistory(date, 365)]);
