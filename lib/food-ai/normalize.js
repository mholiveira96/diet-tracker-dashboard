...
const { getFoodAiConfigs } = require('./config.js');
...
async function normalizeStructuredMealFromFavorites(text = '', profileId = null) {
  ...
  if (profileId) {
    try {
      const profileFavoritesRes = await execute(`SELECT name, calories, protein, carbs, fat, base_amount, unit FROM profile_favorites WHERE profile_id = ?`, [profileId]);
      favorites = profileFavoritesRes.rows || [];
    } catch (error) {
      if (!/no such table|no such column/i.test(error.message || '')) throw error;
    }
    if (!favorites.length) {
      try {
        const legacyFavoritesRes = await execute(`SELECT name, calories, protein, carbs, fat, base_amount, unit FROM favorites WHERE profile_id = ?`, [profileId]);
        favorites = legacyFavoritesRes.rows || [];
      } catch (error) {
        if (!/no such table|no such column/i.test(error.message || '')) throw error;
      }
    }
...
function buildPrompt({ text, attachments }) {
  return [
    ...
    'Para log_meal, inclua sempre meal_items como array. Cada item deve ter description (ou name), amount, unit, calories, protein, carbs e fat. Inclua também o total em description, calories, protein, carbs, fat, amount, unit, confidence e ambiguities.',
...
async function normalizeWithAi({ text = '', attachments = [], config }) {
  ...
  try {
    ...
    const response = await fetch(`${config.baseUrl.replace(/\/$/, '')}/chat/completions`, {
      ...
      headers: {
        ...
        ...(config.provider === 'openrouter'
          ? {
              'HTTP-Referer': 'https://dieta-matheusinho.vercel.app',
              'X-Title': 'Diet Tracker Dashboard',
            }
          : {}),
      },
      body: JSON.stringify({
    ...
    if (parsed?.action === ACTIONS.LOG_MEAL) {
      const items = Array.isArray(parsed.meal_items) ? parsed.meal_items : [];
      const validItems = items.length > 0 && items.every((item) => (
        item && (item.description || item.name) &&
        [item.calories, item.protein, item.carbs, item.fat].every((value) => toNumber(value) !== null)
      ));
      if (!validItems) {
        return {
          action: ACTIONS.CLARIFY,
          confidence: Math.min(toNumber(parsed.confidence) || 0.5, 0.5),
          question: 'Preciso separar essa refeição em itens antes de lançar. Quais alimentos e porções fazem parte dela?',
          ambiguities: ['Resposta da IA não trouxe meal_items válidos para uma refeição segura'],
          source: 'ai-invalid-itemization',
          provider: config.provider,
          model: config.model,
        };
      }
      parsed.meal_items = items.map((item) => ({ ...item, description: item.description || item.name }));
    }
    return {
      ...parsed,
      source: 'ai',
...
async function normalizeUserInput({ text = '', attachments = [], profileId = null }) {
  const structured = profileId ? await normalizeStructuredMealFromFavorites(text, profileId) : null;
  ...
  if (!configs.length) {
    return {
      action: ACTIONS.CLARIFY,
      confidence: 0.2,
      question: 'O parser de IA não está configurado agora. Tenta novamente em instantes.',
      ambiguities: ['Nenhum provedor de IA configurado para o chat'],
  ...
  for (const config of configs) {
    try {
      return await normalizeWithAi({ text, attachments, config });
    } catch (error) {
      attempts.push({ provider: config.provider, model: config.model, error: error.message });
    }
  }

  return {
    action: ACTIONS.CLARIFY,
    confidence: Math.min(heuristic.confidence || 0.35, 0.35),
    question: 'Não consegui usar o parser de IA agora. Tenta de novo em instantes.',
    ambiguities: [
      'Todos os provedores de IA falharam nesta tentativa',
      ...(heuristic.ambiguities || []),
    ],
    source: 'ai-unavailable',
    fallback_attempts: attempts,
    fallback_error: attempts.map((attempt) => `${attempt.provider}: ${attempt.error}`).join(' | '),
  };
}
