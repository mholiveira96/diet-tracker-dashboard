...
function isMobileViewport(windowLike) {
  const viewportMatches = Boolean(windowLike?.matchMedia?.('(max-width: 767px)')?.matches);
  const userAgent = String(windowLike?.navigator?.userAgent || '').toLowerCase();
  return viewportMatches || /android|iphone|ipad|ipod|mobile/.test(userAgent);
}
...
function canShowInstallToast({ standalone, mobile, dismissed, hasDeferredPrompt, ios }) {
  return !standalone && !dismissed && (hasDeferredPrompt || mobile);
}
