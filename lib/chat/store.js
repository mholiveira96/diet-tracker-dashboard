...
async function ensureRollingThread(profileId) {
  const existing = await execute(`SELECT id, title, created_at, updated_at, profile_id FROM chat_threads WHERE profile_id = ? ORDER BY id ASC LIMIT 1`, [profileId]);
  if (existing.rows.length > 0) return existing.rows[0];
  const created = await execute(
    `INSERT INTO chat_threads (title, profile_id, updated_at) VALUES (?, ?, strftime('%Y-%m-%d %H:%M:%S', 'now')) RETURNING id, title, created_at, updated_at, profile_id`,
    ['Diet Tracker', profileId]
  );
...
async function attachUploadsToMessage({ messageId, profileId, attachmentIds = [] }) {
  if (!attachmentIds.length) return;
  const placeholders = attachmentIds.map(() => '?').join(', ');
  const owned = await execute(
    `SELECT a.id FROM chat_attachments a
     JOIN chat_threads t ON t.id = a.thread_id
     WHERE a.id IN (${placeholders}) AND t.profile_id = ? AND a.message_id IS NULL`,
    [...attachmentIds, profileId]
  );
  if (owned.rows.length !== attachmentIds.length) {
    throw new Error('Anexo inválido para o perfil selecionado.');
  }
  await execute(`UPDATE chat_attachments SET message_id = ? WHERE id IN (${placeholders})`, [messageId, ...attachmentIds]);
}
