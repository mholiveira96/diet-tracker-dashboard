...
async function saveParsedAction({ messageId, result, profileId, dbExecute = execute }) {
  if (result.action === 'log_meal') {
    const insert = buildMealInsert(result, profileId);
    ...
    await appendAuditEvent({ profileId, entityType: 'meal', entityId: recordId, action: 'create', after: result, source: 'public_dashboard', dbExecute });
...
async function ingestUserMessage({ threadId, profileId, text = '', attachmentIds = [] }) {
  ...
  if (decision.mode === 'auto_save') {
    const record = await saveParsedAction({ messageId: userMessage.id, result: normalized, profileId });
...
async function confirmDraftMessage(messageId, profileId) {
  const draftRes = await execute(`
    SELECT m.* FROM chat_messages m
    JOIN chat_threads t ON t.id = m.thread_id
    WHERE m.id = ? AND t.profile_id = ? LIMIT 1
  `, [messageId, profileId]);
  ...
  return withTransaction(async (transaction) => {
    ...
    record = await saveParsedAction({ messageId: sourceMessageId, result: normalized, profileId, dbExecute: txExecute });
