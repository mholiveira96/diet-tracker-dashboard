...
function buildDeleteCopy(item) {
  const target = item?.type === 'workout' ? `treino “${item.description}”` : `refeição “${item.description}”`;
  return `Apagar ${target}? Esta ação é pública, ficará registrada no histórico e poderá ser restaurada.`;
}
