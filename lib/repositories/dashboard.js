...
async function getGroupOverview(targetDate = 'now') {
  ...
  const result = await execute(`
    ...
    WHERE p.slug <> 'legacy'
    ORDER BY CASE p.slug WHEN 'matheus' THEN 1 WHEN 'joyce' THEN 2 WHEN 'allan' THEN 3 WHEN 'musi' THEN 4 WHEN 'andreia' THEN 5 WHEN 'marcelo' THEN 6 ELSE 7 END
...
async function getGroupAdherenceHistory(endDate = 'now', windowDays = 365) {
  const dateFilter = buildDateFilter(endDate);
  const daysInWindow = Math.max(7, Math.min(730, Number(windowDays) || 365));
  const result = await execute(`
    WITH RECURSIVE target AS (SELECT date(${dateFilter.sql}) AS end_day),
    calendar(day) AS (
      SELECT date((SELECT end_day FROM target), '-${daysInWindow - 1} days')
      UNION ALL
    ...
    WHERE p.slug <> 'legacy'
    ORDER BY CASE p.slug WHEN 'matheus' THEN 1 WHEN 'joyce' THEN 2 WHEN 'allan' THEN 3 WHEN 'musi' THEN 4 WHEN 'andreia' THEN 5 WHEN 'marcelo' THEN 6 ELSE 7 END, c.day
  `, dateFilter.args);
