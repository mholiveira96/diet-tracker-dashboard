...
async function getDashboardData(targetDate = 'now') {
  ...
  const [summaryRes, goalsRes, itemsRes, historyRes, activityRes, workoutSummaryRes, workoutItemsRes] = await Promise.all([
    ...
    execute(
      `SELECT id, modality, duration_min, calories, workout_type, intensity, notes, logged_at FROM workouts WHERE date(logged_at, '-3 hours') = ${dateFilter.sql} ORDER BY logged_at DESC`,
      dateFilter.args
    ),
  ]);
