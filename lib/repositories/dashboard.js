...
async function getGroupAdherenceHistory(targetDate = 'now', days = 28) {
  const dateFilter = buildDateFilter(targetDate);
  const result = await execute(`
    WITH RECURSIVE target AS (SELECT ${dateFilter.sql} AS end_day),
    calendar(day) AS (
      SELECT date((SELECT end_day FROM target), ?)
      UNION ALL
  ...
  `, [...dateFilter.args, `-${Math.max(1, Number(days) || 28) - 1} days`]);
