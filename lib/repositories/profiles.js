...
const CLAIMABLE_PROFILE_SLUGS = ['matheus', 'joyce', 'allan', 'musi', 'andreia', 'marcelo'];
...
async function listProfiles() {
  const result = await execute(`
    ...
    WHERE slug <> 'legacy'
    ORDER BY CASE slug
      WHEN 'matheus' THEN 1 WHEN 'joyce' THEN 2 WHEN 'allan' THEN 3 WHEN 'musi' THEN 4 WHEN 'andreia' THEN 5 WHEN 'marcelo' THEN 6 ELSE 7
    END
  `);
  return result.rows || [];
}
