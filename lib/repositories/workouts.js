...
async function insertWorkout(profileId, payload) {
  const columns = await getWorkoutColumns();
  const fields = ['modality', 'duration_min', 'calories', 'profile_id'];
  const values = [payload.modality, payload.duration_min, payload.calories, profileId];
  for (const key of ['workout_type', 'intensity', 'notes']) if (payload[key] !== undefined && columns.has(key)) { fields.push(key); values.push(payload[key]); }
  fields.push('logged_at');
  values.push(payload.logged_at || null);
  const placeholders = fields.map((field) => field === 'logged_at' ? "COALESCE(?, strftime('%Y-%m-%d %H:%M:%S', 'now'))" : '?');
  const result = await execute(`INSERT INTO workouts (${fields.join(', ')}) VALUES (${placeholders.join(', ')})`, values);
  return getWorkoutById(Number(result.lastInsertRowid), profileId);
}
...
module.exports = { getWorkoutById, getWorkoutExercisesByWorkoutId, insertWorkout, updateWorkoutById, deleteWorkoutById, restoreWorkoutById, getWorkoutColumns, buildWorkoutSelectList };
