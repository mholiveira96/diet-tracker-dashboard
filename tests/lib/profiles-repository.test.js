...
test('only the closed roster can be claimed from WhatsApp text', () => {
  assert.deepEqual(CLAIMABLE_PROFILE_SLUGS, ['matheus', 'joyce', 'allan', 'musi', 'andreia', 'marcelo']);
  assert.equal(normalizeClaimText('sou Joyce'), 'joyce');
  assert.equal(normalizeClaimText('  SOU   allan '), 'allan');
  assert.equal(normalizeClaimText('sou Andreia'), 'andreia');
  assert.equal(normalizeClaimText('sou Marcelo'), 'marcelo');
